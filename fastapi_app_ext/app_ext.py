...
class AppExt:
        ...
        if app.openapi_url:
            ...
            if app.docs_url:
                ...
                if app.swagger_ui_oauth2_redirect_url:
                    ...
                    app.add_route(path=oauth2_redirect_url, route=self._swagger_ui_redirect, include_in_schema=False)
            # 添加自定义的RedocUI
            if app.redoc_url:
    ...
    def _determine_resource_path(self, resource_type: str, resource_name: str, default_url: str) -> str:
        """
        决定使用本地还是默认URL资源
    ...
    async def _swagger_ui_html(self, r: Request) -> HTMLResponse:
        ...
        swagger_js_url = self._determine_resource_path(
            'js',
            'swagger-ui-bundle.js',
            'https://cdn.jsdelivr.net/npm/swagger-ui-dist@5.9.0/swagger-ui-bundle.js'
        )
        swagger_css_url = self._determine_resource_path(
            'css',
            'swagger-ui.css',
    ...
    @staticmethod
    async def _swagger_ui_redirect(r: Request) -> HTMLResponse:
        """
        SwaggerUIRedirect
        :param r:请求实例
        :return:HTML响应
    ...
    async def _redoc_html(self, r: Request) -> HTMLResponse:
        """
        RedocHTML
        :param r:请求实例
        :return:HTML响应
        ...
        root_path = r.scope.get('root_path', '').rstrip('/')
        openapi_url = root_path + r.app.openapi_url
        redoc_js_url = self._determine_resource_path(
            'js',
            'redoc.standalone.js',
